...
use crate::app::assignment_matches_search;
...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    if !app.assignment_show_all && !authenticated {
        ...
        .block(Block::default().borders(Borders::ALL).title(" Assignments [mine] "));
        f.render_widget(placeholder, area);
        return;
    }

    let all_assignments: &[crate::api::Assignment] = if app.assignment_show_all {
        &session.assignments
    } else {
        &session.my_assignments
    };

    let filtered: Vec<&crate::api::Assignment> = all_assignments
        .iter()
        .filter(|a| {
    ...
    if let Some(sa) = search_area {
        ...
        let cursor = if app.assignment_searching { "\u{2588}" } else { "" };
...
fn render_sidebar(
    ...
    area: Rect,
    app: &App,
    assignments: &[&crate::api::Assignment],
) {
